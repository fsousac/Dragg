...
export function BudgetsScreen() {
  ...
  const chartData = budgetSplitData.map((item) => ({ ...item, name: t(item.nameKey) }))
  ...
  const budgetGroups = [
    ...
    {
      description: t("data.category.entertainment"),
      icon: "🎉",
      key: "wants" as const,
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        {[
          [t("screen.budgets.totalBudget"), formatCurrency(totalBudget), t("screen.budgets.monthlyAllocation")],
          [t("screen.budgets.totalSpent"), formatCurrency(totalSpent), `${Math.round((totalSpent / totalBudget) * 100)}% ${t("common.ofBudget")}`],
          [t("common.remaining"), formatCurrency(remaining), t("screen.budgets.leftToSpend")],
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(value: number) => [`${value}%`, t("screen.budgets.monthlyAllocation")]} />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
            </div>
                ...
                <div key={group.key}>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-foreground">{formatCurrency(group.spent)}</p>
                      <p className="text-xs text-muted-foreground">{formatCurrency(group.budget)}</p>
                    </div>
                  </div>
                  <div
                  ...
                  <div className="mt-1 flex items-center justify-between">
                    <span className={cn("flex items-center gap-1 text-xs", percentage > 100 ? "text-destructive" : "text-muted-foreground")}>
                    ...
                    <span className="text-xs text-muted-foreground">
                      {formatCurrency(group.budget - group.spent)} {t("common.left")}
                    </span>
                  </div>
                </div>
              )
      ...
      <Card className="border-border bg-card card-shadow">
        <CardHeader>
          <CardTitle className="text-lg">{t("screen.budgets.categoryBudgets")}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {categories.filter((category) => category.budget > 0).map((category) => {
              const percentage = Math.round((category.spent / category.budget) * 100)
              return (
                <div key={category.id} className="rounded-lg bg-accent/50 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
